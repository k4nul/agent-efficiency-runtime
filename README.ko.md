...
상태와 실제 사용량 기록도 로컬에 남긴다. 런타임 내부에는 LLM 클라이언트나 모델 API 호출이 없다.
...
| `aer profile`, `aer benchmark` | 실제 기록 집계와 로컬 비교 측정 |
...
프로세스를 종료하고, 그때까지 캡처한 전체 prefix를 secret redaction 후 저장하며
`output_limit_exceeded`를 반환한다. 이 prefix를 완전한 실행 로그라고 표시하지 않는다.
...
스크립트는 `$CODEX_HOME` 또는 `~/.codex`를 탐지하고 `--target`도 받는다. 기존 destination이나
Codex 설정은 덮어쓰지 않는다. 프로젝트별 snippet은 `integrations/codex/AGENTS-snippet.md`다.
...
GUI, provider token 자동 수집을 지원하지 않는다.
